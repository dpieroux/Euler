...
-- elements are equivalent if their image under f is equal.
...
-- A pair of Anagrams. 'min_' comes before 'max_' in lexicographic order. The
-- len_ member is equal to the length of the anagrams and is stored for
-- efficiency.
...
-- Return the Cartesian product of two lists a and b, i.e. all the pairs whose
-- first element is from a and the second from b. The order of the elements is
-- preserved. The first element of a is first associated with each element of b,
-- then the second element of a is associated with each element of b, etc.
cartesianProduct :: [a] -> [b] -> [(a,b)]
cartesianProduct as bs = [(a, b) | a <- as, b <- bs]


-- Check of two pairs of anagrams are similar, i.e. if the second one can be
-- retrieved from the first one under a bijective mapping of the characters.
areSimilar :: AnagramPair -> AnagramPair -> Bool
areSimilar (AnagramPair l n1 n2) (AnagramPair l' w1 w2) 
...
main = do
    ...
    -- List of anagram pairs of the words
    let wordAnagrams = anagrams words
                         
    -- Anagrams pairs of the squares
    let squareAnagrams = anagrams $ map show sqCandidates
                           where
                             bound = (10^) $ maximum $ map len_ wordAnagrams
                             sqCandidates = takeWhile (<bound) $ [n*n | n <- [1..]]

    -- All matching pair of word and square anagrams
    let solution = maximum 
                 $ map (read . max_ . fst)
                 $ filter (\(x, y) -> areSimilar x y)
